...
@get('/find/account')  # or @route('/find/account')
def find_account():
    return '''
        <form action="/find/account" method="post">
            Username: <input name="username" type="text" />
...
@post('/find/account')  # or @route('/find/account', method='POST')
def do_find_account():
    base = Base.Connect('Base')
    username = request.forms.get('username')
    result = Base.FindLogin(base, username)
    base.close()
    if type(result) != int:
        try:
            return str(result[0][0] + '|' + result[0][1] + '|' + str(result[0][2]) + '|' + str(result[0][3]))
        except:
            return "Error"
    else:
        return -6
